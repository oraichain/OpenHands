...
@pytest.mark.asyncio
async def test_cache_history_from_last_user_message_with_cache_enabled(
    mock_agent, mock_event_stream
):
    """Test _cache_history_from_last_user_message when cache is enabled and no previous cache exists."""
    controller = AgentController(
        agent=mock_agent,
        event_stream=mock_event_stream,
        max_iterations=10,
    ...
    # Create test history with user messages and agent actions
    user_msg1 = MessageAction(content='First user message')
    user_msg1._source = EventSource.USER
    user_msg1._id = 1

    agent_msg1 = MessageAction(content='Agent response 1')
    agent_msg1._source = EventSource.AGENT
    agent_msg1._id = 2

    user_msg2 = MessageAction(content='Second user message')
    user_msg2._source = EventSource.USER
    user_msg2._id = 3

    agent_msg2 = MessageAction(content='Agent response 2')
    ...
    cmd_obs = CmdOutputObservation(
        command='ls', content='file1.txt file2.txt', command_id=5
    )
    cmd_obs._id = 6
    cmd_obs._cause = 5
    ...
    mock_cache.async_get_cache.return_value = None  # No existing cache
    mock_cache.async_add_cache = AsyncMock()

    with patch('openhands.controller.agent_controller.litellm') as mock_litellm:
        mock_litellm.cache = mock_cache

        # Call the method
        await controller._cache_history_from_last_user_message()

        # Verify cache was checked for existing entry
        mock_cache.async_get_cache.assert_called_once_with(prompt='Second user message')

        # Verify cache was added with correct data
        mock_cache.async_add_cache.assert_called_once()
        call_args = mock_cache.async_add_cache.call_args
        assert call_args[1]['prompt'] == 'Second user message'

        # Verify the result contains events from the last user message onwards
        result = call_args[1]['result']
        assert len(result) == 4  # user_msg2, agent_msg2, cmd_action, cmd_obs
        assert (
            result[0]['action'] == 'message'
            and result[0]['args']['content'] == 'Second user message'
        )
        assert (
            result[1]['action'] == 'message'
            and result[1]['args']['content'] == 'Agent response 2'
        )
        assert result[2]['action'] == 'run' and result[2]['args']['command'] == 'ls'
        assert (
            result[3]['observation'] == 'run'
            and result[3]['content'] == 'file1.txt file2.txt'
        )
...
@pytest.mark.asyncio
async def test_cache_history_from_last_user_message_already_cached(
    mock_agent, mock_event_stream
):
    """Test _cache_history_from_last_user_message when the prompt is already cached."""
    controller = AgentController(
        agent=mock_agent,
        event_stream=mock_event_stream,
        max_iterations=10,
    ...
    # Create test history
    user_msg = MessageAction(content='Test user message')
    user_msg._source = EventSource.USER
    user_msg._id = 1

    agent_msg = MessageAction(content='Agent response')
    agent_msg._source = EventSource.AGENT
    agent_msg._id = 2

    controller.state.history = [user_msg, agent_msg]

    # Mock litellm.cache to return existing cache
    mock_cache = AsyncMock()
    mock_cache.async_get_cache.return_value = {'cached': 'data'}  # Existing cache
    mock_cache.async_add_cache = AsyncMock()
    ...
    with patch('openhands.controller.agent_controller.litellm') as mock_litellm:
        ...
        mock_cache.async_get_cache.assert_called_once_with(prompt='Test user message')

        # Verify cache was NOT added since it already exists
...
@pytest.mark.asyncio
async def test_cache_history_from_last_user_message_no_cache_enabled(
    mock_agent, mock_event_stream
):
    """Test _cache_history_from_last_user_message when cache is not enabled."""
    controller = AgentController(
        agent=mock_agent,
        event_stream=mock_event_stream,
        max_iterations=10,
    ...
    controller.state.history = [user_msg]

    # Mock litellm.cache to be None
    with patch('openhands.controller.agent_controller.litellm') as mock_litellm:
        ...
        # The method should return early without any cache operations

    await controller.close()


@pytest.mark.asyncio
async def test_cache_history_from_last_user_message_no_user_message(
    mock_agent, mock_event_stream
):
    """Test _cache_history_from_last_user_message when there are no user messages in history."""
    controller = AgentController(
        agent=mock_agent,
        event_stream=mock_event_stream,
        max_iterations=10,
    ...
    # Create test history with only agent actions (no user messages)
    agent_msg = MessageAction(content='Agent response')
    agent_msg._source = EventSource.AGENT
    agent_msg._id = 1
    ...
    controller.state.history = [agent_msg, cmd_action]
    ...
    with patch('openhands.controller.agent_controller.litellm') as mock_litellm:
        ...
        # Verify no cache operations were attempted since there's no user message
        mock_cache.async_get_cache.assert_not_called()
        mock_cache.async_add_cache.assert_not_called()
...
@pytest.mark.asyncio
async def test_cache_history_from_last_user_message_empty_history(
    mock_agent, mock_event_stream
):
    """Test _cache_history_from_last_user_message with empty history."""
    controller = AgentController(
        agent=mock_agent,
        event_stream=mock_event_stream,
        max_iterations=10,
...
@pytest.mark.asyncio
async def test_cache_history_from_last_user_message_cache_exception(
    mock_agent, mock_event_stream
):
    """Test _cache_history_from_last_user_message when cache operations raise exceptions."""
    controller = AgentController(
        agent=mock_agent,
        event_stream=mock_event_stream,
        max_iterations=10,
    ...
    # Mock litellm.cache to raise exception
    mock_cache = AsyncMock()
    mock_cache.async_get_cache.side_effect = Exception('Cache error')
    ...
    with patch('openhands.controller.agent_controller.litellm') as mock_litellm:
        ...
        # The method should log a warning and return without crashing

    await controller.close()


@pytest.mark.asyncio
async def test_cache_history_from_last_user_message_multiple_user_messages(
    mock_agent, mock_event_stream
):
    """Test _cache_history_from_last_user_message with multiple user messages - should cache from the last one."""
    controller = AgentController(
        agent=mock_agent,
        event_stream=mock_event_stream,
        max_iterations=10,
    ...
    # Create test history with multiple user messages
    user_msg1 = MessageAction(content='First user message')
    user_msg1._source = EventSource.USER
    user_msg1._id = 1
    ...
    user_msg3 = MessageAction(content='Third user message')
    user_msg3._source = EventSource.USER
    user_msg3._id = 5

    agent_msg3 = MessageAction(content='Agent response 3')
    agent_msg3._source = EventSource.AGENT
    agent_msg3._id = 6
    ...
    with patch('openhands.controller.agent_controller.litellm') as mock_litellm:
        ...
        # Verify cache was checked for the LAST user message
        mock_cache.async_get_cache.assert_called_once_with(prompt='Third user message')

        # Verify cache was added with events from the last user message onwards
        mock_cache.async_add_cache.assert_called_once()
        call_args = mock_cache.async_add_cache.call_args
        assert call_args[1]['prompt'] == 'Third user message'

        # Verify the result contains only events from the last user message onwards
        result = call_args[1]['result']
        assert len(result) == 2  # user_msg3, agent_msg3
        assert (
            result[0]['action'] == 'message'
            and result[0]['args']['content'] == 'Third user message'
        )
        assert (
            result[1]['action'] == 'message'
            and result[1]['args']['content'] == 'Agent response 3'
        )
...
@pytest.mark.asyncio
async def test_cache_history_from_last_user_message_with_agent_finish_action(
    mock_agent, mock_event_stream
):
    """Test _cache_history_from_last_user_message when the last user message is followed by an AgentFinishAction."""
    controller = AgentController(
        agent=mock_agent,
        event_stream=mock_event_stream,
        max_iterations=10,
    ...
    # Create test history ending with AgentFinishAction
    user_msg = MessageAction(content='Final user message')
    user_msg._source = EventSource.USER
    user_msg._id = 1
    ...
    controller.state.history = [user_msg, finish_action]
    ...
    with patch('openhands.controller.agent_controller.litellm') as mock_litellm:
        ...
        # Verify cache was checked
        mock_cache.async_get_cache.assert_called_once_with(prompt='Final user message')

        # Verify cache was added with both events
        mock_cache.async_add_cache.assert_called_once()
        call_args = mock_cache.async_add_cache.call_args
        assert call_args[1]['prompt'] == 'Final user message'

        # Verify the result contains both events
        result = call_args[1]['result']
        assert len(result) == 2
        assert (
            result[0]['action'] == 'message'
            and result[0]['args']['content'] == 'Final user message'
        )
        assert (
            result[1]['action'] == 'finish'
            and result[1]['args']['outputs']['result'] == 'Task completed'
        )
