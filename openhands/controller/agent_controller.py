...
from openhands.server.human_feedback import HUMAN_FEEDBACK_SYSTEM_PROMPT
    ...
    async def _handle_message_action(self, action: MessageAction) -> None:
        ...
        if action.source == EventSource.USER:
            ...
            if (
                ...
                and not self._has_asked_human_feedback
            ):
                ...
                # Use agent's streaming_llm if available, else create one
                streaming_llm = getattr(self.agent, 'streaming_llm', None)
                if streaming_llm is None:
                    streaming_llm = StreamingLLM(config=self.agent.llm.config)
                ...
                async for chunk in streaming_llm.async_streaming_completion(
                    ...
                    if not delta:
                        continue
                    accumulated_feedback_questions += delta
