...
class OrchestratorAgent(Agent):
    ...
    def _initialize_facts_and_plan(self, state: State) -> None:
        ...
        self.facts = self._call_llm(messages, sid=state.session_id)
        ...
        if self.facts:
            ...
            messages = self._get_messages(state.history, prompt)
            self.plan = self._call_llm(messages, sid=state.session_id)
            logger.info(f'Generated Initial Plan:\n{self.plan}')

    def _initialize_agent(
        self, state: State
    ) -> OrchestratorInitializationAction | AgentFinishAction:
        ...
        """
        initial_action: AgentFinishAction | MessageAction = self._get_initial_task(
            state
        )
        if isinstance(initial_action, AgentFinishAction):
            return initial_action
        self.task = initial_action.content
    ...
    def _get_initial_task(self, state: State) -> AgentFinishAction | MessageAction:
        ...
        if latest_user_message and latest_user_message.content.strip() == '/exit':
            ...
            return AgentFinishAction(
                outputs={'output': 'Exiting due to user request.', 'status': 'success'}
            )
        ...
        if self.user_prompt:
            return MessageAction(content=self.user_prompt)
    ...
    def _call_llm(self, messages: List[Message], sid: str | None = None):
            ...
            for i, tool_call in enumerate(assistant_msg.tool_calls):
                ...
                if tool_call.function.name == LLMBasedFileEditTool['function']['name']:
                    ...
                    if (
                        ...
                        path = (
                            f"{path.rsplit('/', 1)[0]}/{sid}/{path.rsplit('/', 1)[1]}"
                        )
    ...
    def _execute_or_monitor_plan(self, state: State) -> Action:
        ...
        progress_update = self._call_llm(messages, sid=state.session_id)
    ...
    def _update_plan(self, state: State) -> None:
        ...
        messages = self._get_messages(state.history, prompt)
        response_content = self._call_llm(messages, sid=state.session_id)
    ...
    def _update_knowledge(self, state: State) -> Action:
        ...
        facts_response = self._call_llm(messages, sid=state.session_id)
        ...
        plan_response = self._call_llm(messages, sid=state.session_id)
        self.plan = plan_response
        logger.info(f'Updated Plan:\n{self.plan}')

        # After updating both facts and plan, go back to execution with new initialization
        self.phase = OrchestrationPhase.EXECUTING_PLAN
        facts = self.facts or 'No facts available.'
        plan = self.plan or 'No plan available.'
        return OrchestratorInitializationAction(
            task=self.task,
            facts=facts,
            plan=plan,
