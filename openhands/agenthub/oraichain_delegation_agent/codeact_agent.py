...
class OraichainDelegationAgent(Agent):
    VERSION = '2.2'
    """
    The Oraichain Delegation Agent is a specialized agent designed to handle all delegation-related tasks for Oraichain.
    When other agents encounter tasks that require knowledge or actions related to Oraichain delegations, they should delegate those tasks to this agent.

    ### Overview

    This agent serves as the central authority for handling Oraichain delegation operations. It can:

    1. **Converse**: Communicate with other agents or humans to understand delegation requirements
    2. **Delegate**: Process and execute delegation-related tasks including:
    - Managing delegation requests
    - Handling delegation parameters
    - Processing delegation transactions
    - Monitoring delegation status

    The agent maintains specialized knowledge about Oraichain's delegation system and can provide accurate information and perform appropriate actions related to delegations.
    ...
    def __init__(
        ...
        logger.info(
            f"TOOLS loaded for OraichainDelegationAgent: {', '.join([tool.get('function').get('name') for tool in self.tools])}"
        )
    ...
    def reset(self) -> None:
        """Resets the Oraichain Delegation Agent."""
        super().reset()
        self.pending_actions.clear()

    def step(self, state: State) -> Action:
        """Performs one step using the Oraichain Delegation Agent.
        ...
        - McpAction(name, arguments) - MCP action to run
        """
